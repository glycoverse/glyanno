...
The package features five core functions:
...
- `enhance_struc_denovo()`: Reconstructs basic N-glycans at topological resolution from their cores and branches.
...
One last thing to mention before we move on is that you can set custom structure levels for `db`.
...
For basic N-glycans, `enhance_struc_denovo()` reconstructs a single
topological structure directly from the conserved core and observed branches.
If reconstruction is not possible, it uses a topological structure database as
a fallback.
...
| `enhance_struc()` / `enhance_struc_denovo()` | `glyrepr::reduce_structure_level()` |
