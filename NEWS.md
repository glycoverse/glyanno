...
* `comp_to_struc()` now reuses bundled composition metadata for the default structure database, while `enhance_comp()`, `enhance_struc()`, and database fallback in `enhance_struc_denovo()` retain fast batch matching with mixed residues and element-wise structure levels. (#24)
